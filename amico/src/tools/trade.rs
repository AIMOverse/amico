...
fn buy_solana_token(keypair: &Keypair, params: &serde_json::Value) -> ToolResult {
...
pub fn buy_solana_token_tool(keypair: Keypair) -> Tool {
    ToolBuilder::new()
        .name("buy_solana_token")
        .description("Buy a Solana token")
        .parameters(json!({
        "type": "object",
        "properties": {
                "token_address": {
        ...
        .build(move |params| {
            tracing::info!("Calling buy_solana_token tool {:?}", params);
            buy_solana_token(&keypair, &params)
        })
}
